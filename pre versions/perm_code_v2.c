...
/*    gf_t gfs, gfm;
    ...
    gf_t gfs, gfm; 
    int i, j, n, k, m, r, s, w, c, size, nf, p ,q ;
    int L;
    uint8_t **array;
    int *matrix;
    int *lam;
    ...
    n = 5 ;  k=2;  m = (n-k) ;  s = 0 ; r = 4 ; w = 8 ; size = 2 ; IO =0;
    L = (int) round(pow (m,k));
    //   printf("L %02d", l) ;
    printf("size L %02d\n", L);
    lam = talloc(int, k);
...
/*
 ...
 //   if (fpcm == NULL) { perror(argv[8]); exit(1); }
    ...
    print_data(n, L, size, array, "systematic.txt");
    ...
    for (p =0; p<m;  p++) {  // over parity nodes 1:m 
        ...
        //    printf("x_vec after %d ",x_vec[k-1]);
 //           printArray(x_vec_p, 0, k-1, "x_vec_p");
   ...
   print_data(m, L, size, parity, "parity.txt");
    
    
    /* -------  Single failure recovery ---------- */ 
    int *helper_vec;    //
    helper_vec = talloc(int,m) ;
    helper_vec[0] = 1; helper_vec[1] = 1 ; helper_vec[2] = 1 ;
    int helper_dec = basep2dec(helper_vec, 2, m) ;
    int indx_f ;
    uint8_t ** new_node ; 
    int trash1, trash2 ; 
    new_node= talloc(uint8_t *, L);
    for (i = 0; i < L; i++) {
        new_node[i] = talloc(uint8_t, size);
        bzero(new_node[i], size);
    }
    ...
    if (helper_vec == 1 ){  // only parity node 1 is participating for recovery.
        
    }
    else if (helper_dec == 2) {
        
    }
    else if (helper_dec == 3) {
        
    }
    else if (helper_dec == 4) {
        
    }
    else if (helper_dec == 5) {
        
    }
    else if (helper_dec == 6) {
        
    }
    else if (helper_dec == 7) { // all 3 parity nodes participating.
        

        
    }
    else printf("recovery is not possible");
