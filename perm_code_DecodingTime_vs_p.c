...
main(int argc, char **argv)
{
    ...
    n = 5 ;  k =2;  m = (n-k) ; w = 32; IO_read =0; IO_write =1  ;
    itr_max = 1;  // iteration numbers. monte Carlo
    L = (int) round(pow (m,k));
    int max_data = ((int) round(pow(2,w))); 
    /* ------- file size M to block size, size ---- */  
    
    M = 10*288.0/ pow(1024,2);   // in Meg.
 ...
 //               fprintf(stderr,"array %d, temp %d", array[i][j],temp) ;
    ...
    for (i = 0; i < n*L; i++) {
        if (erased[i]) {            // if erased[i] = 1 => sector i is erased.
         //   bzero(array[i], size);  // set the first size * ((int) round(w/8)) bytes to zero
            Init_zero(array[i],size) ;
        }
    }
    ...
    for (i = 0; i < m*L; i++) {
        Init_zero(parity[i],size) ;
      //  bzero(parity[i], size);  // The bzero(s, n) function shall place n zero-valued bytes in the area pointed to by s.
    }
            ...
            for (i = 0; i < k; i++) {
              ...
              //  printArray(x_vec_p, 0, k-1, "x_vec_p");
                ...
                {    //
                 ...
                 //   gfm.multiply_region.w32(&gfm, 9380, tot, 1, 1, 1);
                }


                // it is summed over
        //        fprintf(stderr, "parity: %d, array: %d",parity[indx][0], array[indx_hat][0]);
         //       fprintf(stderr, "parity: %d, array: %d",parity[indx][1], array[indx_hat][1]);
              //  fprintf(stderr, "parity: %d, coef: %d,  array: %d",tot, 1, 9380);
...
//      fprintf(stderr, "timer %f", timer) ;
        ...
        if (helper_vec[p] ==1 ) {
            ...
            {   Init_zero(x_vec, k) ;
                ...
                for (i =1; i<k; i++ ) { // cancell the effect of all systematic nodes on each row of parity nodes.
                    ...
                    //
                    fprintf(stderr, "parity: %d, array: %d",new_node[indx_f][1], array[indx_hat][1]);
                }
  ...
  //    fprintf(stderr, "split: %f \n", split) ;
      ...
      RecoveryTime[pnum-1] +=  split/itr_max  ; //+ RecoveryTime[pnum-1]; //taking average
