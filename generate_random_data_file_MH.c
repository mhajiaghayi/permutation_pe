...
{
  int i, j, n, m, r, size;   // size of each bock in bytes 
  long seed;
  int *erased;

  if (argc != 6) usage("Wrong number of arguments");
  ...
  if (sscanf(argv[3], "%d", &r) == 0 || r <= 0) usage("Bad r");
  if (sscanf(argv[4], "%d", &size) == 0 || size <= 0) usage("Bad size");
  if (sscanf(argv[5], "%ld", &seed) == 0) usage("Bad seed");
  ...
  for (i = 0; i < m; i++) for (j = 0; j < r; j++) {
    erased[n-i-1+j*n] = 1;   // erased is just a vector here. set the coding nodes to 1. others zero. 
  }
  j = n*r-1;
  ...
  for (i = 0; i < n*r; i++) {
    if (erased[i]) {
      ...
      for (j = 0; j < size; j++) printf("%02lx ", lrand48()%256);
        ...
        // %02lx : %[flags(0)][width (2)][.precision][length(l)]specifier (x: hexadecimal)
    }
    printf("\n");
  }
  return 0;
